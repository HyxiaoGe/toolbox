...
import os
import sys
import subprocess # 用于运行 ffprobe -version
import datetime # 用于时间戳

# 调整 sys.path 以包含项目根目录，以便导入后端模块
# __file__ 是 gui_app/tools/tool_video_duration_stats.py
...
class ToolPluginFrame(ctk.CTkFrame):
    ...
    def __init__(self, master):
        ...
        self.grid_rowconfigure(3, weight=1) # 用于日志区域
        ...
        # Log Display Area (using scrolledtext for consistency with other tools)
        ...
        # 如果首次加载时 ffprobe 路径有效（可选，或在成功运行时），则在配置中更新它
        # 让我们以静默方式执行初始验证，或者如果从配置中发现无效则记录日志
        ...
        elif os.path.isfile(self.ffprobe_path_var.get()): # 如果它是一个文件并且上面没有验证通过（例如静默模式使其通过）
             # 如果有效则重新检查并保存（这涵盖了首次加载有效默认值的情况）
    ...
    def _validate_ffprobe_path(self, ffprobe_exe_path, silent=False):
        if not ffprobe_exe_path or not os.path.isfile(ffprobe_exe_path):
            ...
            return False
        try:
            # 尝试获取版本信息。CREATE_NO_WINDOW 用于 Windows，以避免控制台窗口闪烁。
            result = subprocess.run(
                [ffprobe_exe_path, "-version"],
                capture_output=True, text=True, check=False, # check=False 以便手动处理非零退出码
                creationflags=subprocess.CREATE_NO_WINDOW if sys.platform == "win32" else 0
            )
            # ffprobe -version 通常输出到 stdout。它应包含 "ffprobe version" 或 "ffmpeg version"
            # （某些构建可能是完整 ffmpeg 软件包的一部分）。
            output = result.stdout.strip() + result.stderr.strip() # 合并两者，某些版本会输出到 stderr
            if result.returncode == 0 and ("ffprobe version" in output.lower() or "ffmpeg version" in output.lower()):
                ...
                return False
        except FileNotFoundError: # 应由 os.path.isfile 捕获，但作为备用
            if not silent:
    ...
    def _apply_theme_to_scrolledtext(self, text_widget):
        ...
        try:
            ...
            text_widget.config(
                ...
                padx=5, pady=5,
                state=tk.DISABLED # 初始状态
            )
        except Exception:
            text_widget.config(font=("Arial", 11), relief=tk.SOLID, borderwidth=1, state=tk.DISABLED) # 基本回退
            pass
    ...
    def browse_ffprobe(self):
        ...
        if file_path:
            ...
            if self._validate_ffprobe_path(file_path):
                 ...
                 # self.log_message(f"FFprobe 路径已更新并保存: {file_path}") # 验证成功时会记录日志
            # else: # 验证失败时会记录日志
                 # self.log_message(f"选择的 FFprobe 路径无效: {file_path}", is_error=True)
    ...
    def calculate_duration_action(self):
        ...
        # 如果 ffprobe_path 有效、与已保存的不同，并且通过 _validate_ffprobe_path 验证，则将其保存到配置
        current_saved_path = get_setting(ToolPluginFrame.TOOL_NAME, ToolPluginFrame.CONFIG_KEY_FFPROBE_PATH)
        if ffprobe_path != current_saved_path: # 无需再次检查 os.path.isfile，_validate_ffprobe_path 已经做过了。
        ...
        try:
            ...
            for msg in log_messages:
                self.log_message(msg) # 后端现在返回一个日志列表，追加它们
        
            # 后端已经将摘要附加到 log_messages，因此我们可能不需要在此处重复记录。
            # 但是，我们仍然需要格式化的时长用于消息框。
            formatted_total_duration = format_duration_to_hhmmss(total_seconds)
            
            # 为消息框构建摘要，后端 log_messages 已有其自己的摘要。
